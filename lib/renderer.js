...
export type RenderPropsType = {
  text: string,
  data?: Object,
  render?: ({ translatedText: string }) => React.Node,
...
export const create: CreateType = ({ Component }) => {
  ...
  const renderWithRenderers = basicParser({ renderers })
  return ({
    text,
    data,
    render,
    ...
    if (render) {
      return render({ translatedText: template })
    }

    const chunks = renderWithRenderers({
